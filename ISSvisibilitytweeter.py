...
def find_best_sighting(city, state):
    ...
    def clear_sky():
        ...
        #print('The #ISS will be visible from #' + city.capitalize() + ' tomorrow, ' + weekday + ' ' + tomorrow.strftime('%B %d') + ' at ' + date.strftime(best_time, '%H:%M %p') + ', for ' + longest_time + ' min.\n\n' + 'Appears: ' + appears + '\nDisappears: ' + disappears)
    ...
    while current_item != weather_len:
        if int(weather_json['list'][current_item]['dt']) > unix_time:
            ...
            # uncomment print for local weather description
